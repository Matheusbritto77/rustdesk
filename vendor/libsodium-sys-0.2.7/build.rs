...
fn make_libsodium(target: &str, source_dir: &Path, install_dir: &Path) -> PathBuf {
    if target.contains("windows-msvc") || target.contains("windows-gnu") {
        return get_lib_dir_for_target(target);
    }
...
#[cfg(not(windows))]
fn get_lib_dir_for_target(target: &str) -> PathBuf {
    ...
    if target.contains("windows-msvc") {
        ...
        crate_dir.join("msvc").join(arch).join(profile).join("v142")
    } else if target.starts_with("i686") {
        crate_dir.join("mingw/win32")
    } else {
        crate_dir.join("mingw/win64")
    }
}
